...
for kk = 1:2
    ...
    for ii = 1:3
        ...
        display([ROIs{ii} ' hits t test results'])
        ChanMat{ii} = data.BinmHits(hemROIChans{kk,ii},BOT);
        ttestPrint(ChanMat{ii},MAIN_alpha,time,center)
        
        display([ROIs{ii} ' CRs t test results'])
        ChanMat{ii} = data.BinmCRs(hemROIChans{kk,ii},BOT);
        ttestPrint(ChanMat{ii},MAIN_alpha,time,center)
    end
    % roi interactions
    for ii = 1:2
